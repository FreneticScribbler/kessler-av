...
from .io import Command, Instruction, SerialDevice


class MediaSwitch(MediaSwitchProtocol):
    def __init__(self, device: SerialDevice, machine_id: Optional[int] = None):
        self._device = device
    ...
    def select_source(self, input: int, output: int) -> None:
        """
        Select the specified video input
        """
        normalized_input = self.normalize_value(input)
        normalized_output = self.normalize_value(output)
